...
variable "use_custom_name" {
  type        = bool
  default     = false
  description = "Set to true to use `custom_name` directly as the resource ID, bypassing the label generation logic"
}

variable "custom_name" {
  type        = string
  default     = ""
  description = "Custom name to use as the resource ID when `use_custom_name` is true"
}
...
variable "protected_subnets" {
  description = "A list of protected subnets inside the VPC (no external access allowed)"
  type        = list(string)
  default     = []
}
...
variable "protected_subnet_enable_dns64" {
  description = "Indicates whether DNS queries made to the Amazon-provided DNS Resolver in this subnet should return synthetic IPv6 addresses for IPv4-only destinations. Default: `true`"
  type        = bool
  default     = true
}

variable "protected_subnet_enable_resource_name_dns_aaaa_record_on_launch" {
  description = "Indicates whether to respond to DNS queries for instance hostnames with DNS AAAA records. Default: `true`"
  type        = bool
  default     = true
}

variable "protected_subnet_enable_resource_name_dns_a_record_on_launch" {
  description = "Indicates whether to respond to DNS queries for instance hostnames with DNS A records. Default: `false`"
  type        = bool
  default     = false
}

variable "create_multiple_protected_route_tables" {
  description = "Indicates whether to create a separate route table for each protected subnet. Default: `false`"
  type        = bool
  default     = false
}

variable "protected_subnet_ipv6_prefixes" {
  description = "Assigns IPv6 protected subnet id based on the Amazon provided /56 prefix base 10 integer (0-256). Must be of equal length to the corresponding IPv4 subnet list"
  type        = list(string)
  default     = []
}

variable "protected_subnet_ipv6_native" {
  description = "Indicates whether to create an IPv6-only subnet. Default: `false`"
  type        = bool
  default     = false
}

variable "protected_subnet_private_dns_hostname_type_on_launch" {
  description = "The type of hostnames to assign to instances in the subnet at launch. For IPv6-only subnets, an instance DNS name must be based on the instance ID. For dual-stack and IPv4-only subnets, you can specify whether DNS names use the instance IPv4 address or the instance ID. Valid values: `ip-name`, `resource-name`"
  type        = string
  default     = null
}

variable "protected_subnet_names" {
  description = "Explicit values to use in the Name tag on protected subnets. If empty, Name tags are generated"
  type        = list(string)
  default     = []
}
...
variable "protected_dedicated_network_acl" {
  description = "Whether to use dedicated network ACL (not default) and custom rules for protected subnets"
  type        = bool
  default     = false
}
...
variable "protected_acl_tags" {
  description = "Additional tags for the protected subnets network ACL"
  type        = map(string)
  default     = {}
}
